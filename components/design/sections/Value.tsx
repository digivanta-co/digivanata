...
export function BigStatement({ lines = ["Every pixel", "tells a story."] }: { lines?: string[] }) {
  ...
  return (
    <section ref={root} className="flex items-center border-y border-[var(--gd-line)] bg-[var(--gd-soft)] py-8 sm:py-12">
                    ...
                    <span className="inline-block whitespace-nowrap">
                      {Array.from(word).map((ch, ci) => (
                        <span key={ci} className="inline-block overflow-hidden">
                          <span className="gd-stmt__char inline-block">{ch}</span>
                        </span>
                      ))}
                    </span>
                    {wi < arr.length - 1 && " "}
